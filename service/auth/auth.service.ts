...
import { changePasswordSchema, forgotPasswordSchema, resetPasswordSchema } from "@/validation/zod/auth/auth.validation";
...
export async function resetPassword(_prevState: any, formData: FormData) {
    ...
    // Validate
    const validatedPayload = zodValidate(
        validationPayload,
        resetPasswordSchema
    );
    ...
    try {
        ...
        if (token) {
            jwt.verify(token, envVars.jwt.jwt_reset_pass_secret as string);
        }
